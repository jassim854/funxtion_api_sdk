...
class FitnessActivityTypeRequest {
  Future<List<FitnessActivityTypeModel>?> getFtinessActivitiesType({ Duration? maxAge ,bool?forceRefresh,Duration? maxStale}) async {
    ...
    try {
      ...
      var response = await netwoerkHelper.getFitnessActivitiesTypeRequest(
        ...
        forceRefresh: forceRefresh??true,maxStale: maxStale
      );
...
{required String id, Duration? maxAge ,bool?forceRefresh,Duration? maxStale}) async {
    ...
    try {
      _dioCacheManager = DioCacheManager(CacheConfig());
      netwoerkHelper.dio.interceptors.add(_dioCacheManager?.interceptor);
      var response = await netwoerkHelper.getFitnessActivityTypeByIdrequest(id: id,maxAge: maxAge??const Duration(days: 7),forceRefresh: forceRefresh??true,maxStale: maxStale);
      if (response.statusCode == 200) {
